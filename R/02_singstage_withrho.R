...
#'
#' @export
optimalSingleTrial_EU <- function(lambda.2, w.2,
                                   x, r,
                                   ...
                                   verbose = TRUE, tol.zero = 1e-10, forOpt = TRUE){
  ...
  crit_w.2_1 = qnorm(1 - alpha * w.2_1)
  crit_w.2_2 = qnorm(1 - alpha * (1-w.2_1))
  crit       = qnorm(1 - alpha)

  theta_crit_w.2_1 = crit_w.2_1 * sqrt(sigma2_theta.2_1)
  theta_crit_w.2_2 = crit_w.2_2 * sqrt(sigma2_theta.2_2)
  theta_crit_1 = crit * sqrt(sigma2_theta.2_1)
  theta_crit_2 = crit * sqrt(sigma2_theta.2_2)
  ...
  if (isTwoStageTrial & (0 < lambda.2_1 & lambda.2_1 < 1)){
    ...
    q1 <- mvtnorm::pmvnorm(lower = c(-Inf,             theta_crit_w.2_2),
                           upper = c(theta_crit_1,     Inf),
                           mean  = mu.1,
                           sigma = Sigma_ppd,
                           algorithm = mvtnorm::GenzBretz())[1]
    q2 <- mvtnorm::pmvnorm(lower = c(theta_crit_1,     theta_crit_w.2_2),
                           upper = c(theta_crit_w.2_1, Inf),
                           mean  = mu.1,
                           sigma = Sigma_ppd,
                           algorithm = mvtnorm::GenzBretz())[1]
    q3 <- mvtnorm::pmvnorm(lower = c(theta_crit_w.2_1, theta_crit_w.2_2),
                           upper = c(Inf,              Inf),
                           mean  = mu.1,
                           sigma = Sigma_ppd,
                           algorithm = mvtnorm::GenzBretz())[1]
    q4 <- mvtnorm::pmvnorm(lower = c(theta_crit_w.2_1, theta_crit_2),
                           upper = c(Inf,              theta_crit_w.2_2),
                           mean  = mu.1,
                           sigma = Sigma_ppd,
                           algorithm = mvtnorm::GenzBretz())[1]
    q5 <- mvtnorm::pmvnorm(lower = c(theta_crit_w.2_1, -Inf),
                           upper = c(Inf,              theta_crit_2),
                           mean  = mu.1,
                           sigma = Sigma_ppd,
                           algorithm = mvtnorm::GenzBretz())[1]
    q6 <- mvtnorm::pmvnorm(lower = c(theta_crit_1,     theta_crit_2),
                           upper = c(theta_crit_w.2_1, theta_crit_w.2_2),
                           mean  = mu.1,
                           sigma = Sigma_ppd,
                           algorithm = mvtnorm::GenzBretz())[1]
  ...
  if(forOpt){
    ...
    out = c(EU = EU, pr1 = pr1, pr2 = pr2, pr12 = pr12)
    return(out)
  }
}


grid_search_single <- function(lower = c(0,0), upper = c(1,1),
                        grid_points = 20,
                        x, r,
                        sigma_1 = 1, sigma_2 = 1, rho = 0,
                        mu_1 = 0, psi_1 = 1,
  ...
  results = sapply(X = 1:nrow(grid),
                   FUN = function(xx){
                     optimalSingleTrial_EU(lambda.2 = grid[xx,1],
                                            w.2      = grid[xx,2],
                                            x = x,
                                            r = r,
                                            rho = 0,
  ...
  # results.df
  grid_max = which.max(results)
  list(par = unname(unlist(grid[grid_max, ])),
       value = results[[grid_max]],
...
#'
#' @export
optimalSingleTrial = function(x, r,
                               sigma_1 = 1, sigma_2 = 1, rho = 0,
                               mu_1 = 0, psi_1 = 1,
                               mu_2 = 0, psi_2 = 1, rho0 = 0,
                               lambda = 0.5, lambda.1 = 0.5,
  ...
  if(optim_func == "optim"){
  out = optimr::optimr(par = starting_points,
                fn = function(y, ...) optimalSingleTrial_EU(lambda.2 = y[1],
                                                            w.2      = y[2], ...),
                x = x, r = r,
    ...
    out = dfoptim::hjkb(par = starting_points,
                        fn = function(y, ...) optimalSingleTrial_EU(lambda.2 = y[1], w.2 = y[2], ...),
                        x = x, r = r,
                        sigma_1 = sigma_1, sigma_2 = sigma_2, rho = rho,
                        mu_1 = mu_1, psi_1 = psi_1,
                        mu_2 = mu_2, psi_2 = psi_2, rho0 = rho0,
    ...
    out = grid_search_single(x = x, r = r,
                      sigma_1 = sigma_1, sigma_2 = sigma_2, rho = rho,
                      mu_1 = mu_1, psi_1 = psi_1,
                      mu_2 = mu_2, psi_2 = psi_2, rho0 = rho0,
                      lambda = lambda, lambda.1 = lambda.1,
    ...
    out = Rvmmin::Rvmmin(par = starting_points,
                        fn = function(y, ...) (-1)*optimalSingleTrial_EU(lambda.2 = y[1], w.2 = y[2], ...),
                        x = x, r = r,
                        sigma_1 = sigma_1, sigma_2 = sigma_2, rho = rho,
                        mu_1 = mu_1, psi_1 = psi_1,
                        mu_2 = mu_2, psi_2 = psi_2, rho0 = rho0,
    ...
    out = dfoptim::nmkb(par = starting_points,
                        fn = function(y, ...) optimalSingleTrial_EU(lambda.2 = y[1], w.2 = y[2], ...),
                        x = x, r = r,
                        sigma_1 = sigma_1, sigma_2 = sigma_2, rho = rho,
                        mu_1 = mu_1, psi_1 = psi_1,
...
#'   if (r.1 != 1){
#'   interimDecision = optimalSingleTrial(x = theta.hat.1,
#'                                         r = r.1,
#'                                         sigma_1 = 1, sigma_2 = 1,
#'                                         rho  = 0,
#'                                         mu_1 = mu_1, psi_1 = psi_1,
...
#'                           tol.zero =   0.00001,
#'                           tol.lim  =   0.0001,
#'                           verbose  = TRUE, summarize = TRUE, optim_func = "hjkb"){
#'   # Simulate 1st stage estimates
...
#'
#' #'@export
#' summary.simOptimalTrial <- function(obj, verbose = FALSE){
#'   results.df = obj$simresults
